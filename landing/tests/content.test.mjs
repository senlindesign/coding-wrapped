...
test("how it works uses a dedicated character sprite sequence", async () => {
  const source = await readFile(new URL("../src/App.jsx", import.meta.url), "utf8");
  assert.match(source, /how-it-works-story-sprite-v7\.webp/);
  assert.doesNotMatch(source, /Coding Wrapped app icon showing a robot reading a notebook/);
  await readFile(new URL("../public/assets/how-it-works-story-sprite-v7.webp", import.meta.url));
});

test("supporting illustrations use real six-frame character sprite stories", async () => {
  const source = await readFile(new URL("../src/App.jsx", import.meta.url), "utf8");
  const styles = await readFile(new URL("../src/styles.css", import.meta.url), "utf8");
  const builder = await readFile(new URL("../scripts/register_action_sprites.py", import.meta.url), "utf8");
  ...
  assert.match(source, /practice-tips-story-sprite-v5\.webp/);
  ...
  assert.match(builder, /sequence lacks genuine pose changes/);
  assert.match(builder, /transparent padding is missing/);
...
test("practice tips module explains provenance and links the source-of-truth library", async () => {
  const source = await readFile(new URL("../src/App.jsx", import.meta.url), "utf8");
  const styles = await readFile(new URL("../src/styles.css", import.meta.url), "utf8");
  const image = await stat(new URL("../public/assets/practice-tips-story-sprite-v5.webp", import.meta.url));
...
test("large below-fold images are lazy, async decoded, and dimensioned", async () => {
  ...
  assert.match(source, /loading="lazy"[\s\S]*how-it-works-story-sprite-v7\.webp/);
  assert.match(source, /loading="lazy"[\s\S]*practice-tips-story-sprite-v5\.webp/);
